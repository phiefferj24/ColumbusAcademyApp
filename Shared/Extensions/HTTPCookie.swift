...
extension HTTPCookie {
    func store(_ key: String) {
        guard let storedData = UserDefaults.standard.object(forKey: key) as? Data else { return }
    ...
    static func get(_ key: String) -> [HTTPCookie]? {
        guard let storedData = UserDefaults.standard.object(forKey: key) as? Data else { return nil }
